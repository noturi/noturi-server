...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description: 'Native 클라이언트에서 받은 Apple identityToken으로 로그인합니다.',
  })
  @ApiResponse({ status: 200, description: '로그인 성공', type: LoginResponse })
  @ApiBody({
    description: 'Apple Identity Token 및 사용자 정보',
    schema: { 
      type: 'object', 
      properties: { 
  ...
  async appleNativeAuth(@Body() body: { identityToken: string; appleId: string; email: string; name?: string; user: string }) {
    try {
